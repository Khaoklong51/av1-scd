...
def get_print_final() -> str:
    ...
    if enc_format == ALL_CFG_OPT[0]: # x264
        final_help = f"Feed the config file to {enc_format} using --qpfile with '--keyint infinite --no-scenecut' option"
    elif enc_format == ALL_CFG_OPT[1]: # x265
        final_help = f"Feed the config file to {enc_format} using --qpfile with '--keyint -1 --no-scenecut' option"
    elif enc_format == ALL_CFG_OPT[2]: # svt-av1
        final_help = f'Feed the config file to SvtAv1EncApp using -c or --config with --keyint -1 option'
    elif enc_format == ALL_CFG_OPT[3] or enc_format == ALL_CFG_OPT[4]: # av1an and av1an-git
        final_help = f'Feed the scene file to av1an using -s or --scenes option'
    elif enc_format == ALL_CFG_OPT[5]: # ffmpeg
        final_help = f'Feed the content of config file to -force_key_frames:v option in ffmpeg also make sure to disable keyframe placement with the encoder you use'
...
def main():
    ...
    if scd_method == ALL_SCD_METHOD[0]:
        ...
        keyframe_list = avscenechange.get_keyframe_avscenechange(input_file, pix_fmt, mediainfo.get_frame_count(track_data))

    keyframe_list = keyframes.process_keyframe(keyframe_list, mediainfo.get_frame_count(track_data))
